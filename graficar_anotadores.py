...
def geragrafico(caminho_arquivo, nome):
    ...
    if class_distribution:
        ...
        plt.title('Distribuição das classes do radiologista '+ nome + " em " + str(total_images) + " imagens com total de " + str(total_annotations) + ' anotações', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Gera gráficos de distribuição de classes por anotador.')
    parser.add_argument('--pasta', default='anotadores', help='Pasta com os arquivos TXT dos anotadores')
    args = parser.parse_args()

    caminho_pasta = Path(args.pasta)
